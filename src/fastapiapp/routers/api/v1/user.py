...
def read_users(limit: int = 100, db: Session = Depends(get_db)):
    users = cruds_user.get_users(db, limit=limit)
    return users

@user_router.get('/user/{user_id}', response_model=User)
def read_user(user_id: int, db: Session = Depends(get_db)):
    db_user = cruds_user.get_user(db, user_id=user_id)
    if not db_user:
        raise HTTPException(status_code=404, detail='User not found')
    return db_user
...
def create_user(user: UserCreate, db: Session = Depends(get_db)):
    return cruds_user.create_user(db=db, user=user)


@user_router.put('/users/{user_id}', response_model=UserUpdate)
def update_user(user_id: int, user: UserUpdate, db: Session = Depends(get_db)):
    return cruds_user.update_user(db=db, user_id=user_id, user=user)


@user_router.delete('/users/{user_id}')
def delete_user(user_id: int, db: Session = Depends(get_db)):
    cruds_user.delete_user(db=db, user_id=user_id)
...
def read_userprofile(userprofile_id: int, db: Session = Depends(get_db)):
    db_userprofile = cruds_user.get_userprofile(db, userprofile_id=userprofile_id)
...
def update_userprofile(userprofile_id: int, userprofile: UserProfileUpdate, db: Session = Depends(get_db)):
    return cruds_user.update_userprofile(db=db, userprofile_id=userprofile_id, userprofile=userprofile)
